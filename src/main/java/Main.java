...
public class Main {
    ...
    static byte[] handleForwarding(byte[] requestData, String resolverAddress, DatagramSocket socket) {
        try {
            ...
            byte[] responseData = Arrays.copyOfRange(response.getData(), 0, response.getLength());

            // Modify header flags
            ByteBuffer buffer = ByteBuffer.wrap(responseData).order(ByteOrder.BIG_ENDIAN);
            short flags = buffer.getShort(2); // Get current flags
            flags |= (1 << 15); // Set QR to 1 (response)
            buffer.putShort(2, flags); // Update flags in the buffer
